 ...
 *
 */
class Tx_Fed_ViewHelpers_Be_ContainerViewHelper extends Tx_Fluid_ViewHelpers_Be_AbstractBackendViewHelper {
	 ...
	 * @param string $addCssFile Custom CSS files to be loaded. Use CSV-format to include several files.
	 * @param string $addJsFile Custom JavaScript files to be loaded. Use CSV-format to include several files.
	 * @return string
	 * @see template
	...
	public function render($pageTitle = '', $enableJumpToUrl = TRUE, $enableClickMenu = TRUE, $loadPrototype = TRUE, $loadScriptaculous = FALSE, $scriptaculousModule = '', $loadExtJs = FALSE, $loadExtJsTheme = TRUE, $extJsAdapter = '', $enableExtJsDebug = FALSE, $addCssFile = NULL, $addJsFile = NULL) {
		...
		if ($addCssFile !== NULL) {
			$cssFiles = t3lib_div::trimExplode(",", $addCssFile);
			foreach($cssFiles as $cssFile) {
				$pageRenderer->addCssFile($cssFile);
			}
		}
		if ($addJsFile !== NULL) {
			$jsFiles = t3lib_div::trimExplode(",", $addJsFile);
